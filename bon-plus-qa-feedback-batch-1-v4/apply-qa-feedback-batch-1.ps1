...
function Replace-Exact {
    ...
    # Fallback: whitespace-tolerant match. This handles files that were formatted
    # differently (indentation/newlines) without depending on an exact text block.
    $tokens = [regex]::Split($Old.Trim(), '\s+') | Where-Object { $_.Length -gt 0 }
    if ($tokens.Count -eq 0) {
        throw "Patch point is empty: $Label"
    }
    ...
    if (-not $match.Success) {
        throw "Patch point not found: $Label. Your local file may differ from the expected project structure."
    }
